...
<div class="student-group-form">
    <form class="row" method="GET" action="{{ route('admin.teacher.index') }}">
        <div class="col">
...
@push('script')
    <script>
        ...
        $('#guruTable').DataTable({
            ajax: {
                url: "{{ route('admin.teacher.get-data') }}",
                type: 'GET',
                dataType: 'json',
                dataSrc: 'data'
            },
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        var editUrl = '{{ route('admin.teacher.edit', ':id') }}'.replace(':id', full.id);
                        var deleteUrl = '{{ route('admin.teacher.destroy', ':id') }}'.replace(':id', full
                            .id);
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        ...
                        if (confirmResult.isConfirmed) {
                            $.ajax({
                                url: `{{ route('admin.teacher.destroy.all') }}`,
                                type: 'DELETE',
        ...
        $('#guruTable').on('click', '.btn-delete', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ route('admin.teacher.destroy', ':id') }}`.replace(':id', guruId),
                        type: 'DELETE',
                        success: function(response) {
                            $('#guruTable').DataTable().ajax.reload();
                            Swal.fire({
